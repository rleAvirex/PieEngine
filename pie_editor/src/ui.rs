...
use egui::{
    CentralPanel, Color32, Context, CornerRadius, Frame, Image, Key, Margin, Sense, SidePanel,
    Stroke, TextureId, TopBottomPanel, load::SizedTexture, vec2,
};
use hecs::Entity;
            ...
            ui.horizontal(|ui| {
                ...
                let play_btn = if is_running {
                    ...
                    egui::Button::new(RichText::new("  >  ").color(ACCENT_PLAY).size(12.0))
                        .corner_radius(ROUNDING_SM)
                        .fill(ACCENT_PLAY.linear_multiply(0.1))
                ...
                let step_btn =
                    egui::Button::new(RichText::new(" >| ").color(TEXT_SECONDARY).size(12.0))
                        .corner_radius(ROUNDING_SM)
                        .fill(BG_WIDGET)
                ...
                let reload_btn =
                    egui::Button::new(RichText::new("  R  ").color(TEXT_SECONDARY).size(12.0))
                        .corner_radius(ROUNDING_SM)
                        .fill(BG_WIDGET)
                        .stroke(Stroke::new(1.0, BORDER_SUBTLE));
                if ui
    ...
    SidePanel::left("outliner")
        ...
        .show(ctx, |ui| {
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ui.label(
                        RichText::new(format!("Meshes: {mesh_count}"))
                            .color(TEXT_DIM)
                            .size(9.0),
                    );
                    ui.label(
                        RichText::new(format!("Mats: {material_count}"))
                            .color(TEXT_DIM)
                            .size(9.0),
                    );
                    ui.label(
                        RichText::new(format!("Tex: {texture_count}"))
                            .color(TEXT_DIM)
                            .size(9.0),
                    );
                });
            ...
            for entity in entities {
                ...
                let item_stroke = if selected {
                    Stroke::new(1.0, ACCENT_PRIMARY.linear_multiply(0.4))
                } else {
                    Stroke::NONE
                };
                    ...
                    ui.horizontal(|ui| {
                        ui.label(
                            RichText::new(if selected { ">" } else { " " })
                                .color(if selected { ACCENT_PRIMARY } else { TEXT_DIM })
                                .size(10.0)
                                .monospace(),
                        );
                        if ui
                            .label(RichText::new(&name).color(text_color).size(11.0))
                            .clicked()
                        {
            ...
            if let Some(entity) = *selected_entity {
                ...
                .show(ui, |ui| {
                    ui.vertical_centered(|ui| {
                        ...
                        ui.add_space(SPACING_XS);
                        ui.label(
                            RichText::new("Click an entity in the viewport")
                                .color(TEXT_DIM)
                                .size(10.0),
                        );
                    });
        ...
        .show(ctx, |ui| {
            ...
            if let Some(texture_id) = viewport_texture_id {
                let border_stroke = if commands.viewport_hovered {
                    Stroke::new(1.0, VIEWPORT_BORDER_ACTIVE)
                } else {
            ...
            ui.horizontal(|ui| {
                ...
                if let GizmoState::Dragging { axis, .. } = gizmo_state {
                    ...
                    let (al, ac) = match axis {
                        Axis::X => ("X", ACCENT_DANGER),
                        Axis::Y => ("Y", ACCENT_SUCCESS),
                        Axis::Z => ("Z", ACCENT_PRIMARY),
                    };
                    ui.label(
                        RichText::new(format!("Drag {al}"))
                            .color(ac)
                            .size(10.0)
                            .strong(),
                    );
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ui.label(
                        RichText::new("60 Hz  |  Space: Play  N: Step  R: Reload")
                            .color(TEXT_DIM)
                            .size(9.0),
                    );
                });
